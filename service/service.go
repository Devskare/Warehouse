...
func ProductADD(ctx context.Context, conn *pgx.Conn, product models.ProductModel) error {
	...
	sqlQuery := `INSERT INTO products ( article, product_name, storage_id , delivery_date, expire_date, weight)
    Values ( $1, $2, $3, $4, $5, $6) `

	_, err := conn.Exec(
		ctx, sqlQuery, product.Article, product.ProductName, product.StorageID, product.DeliveryDate, product.ExpireDate, product.Weight)
	if err != nil {
		return err
	}
	return nil
...
func ProductDelete(ctx context.Context, conn *pgx.Conn, product models.ProductModel) error {
	...
	_, err := conn.Exec(ctx, sqlQuery, product.Article)
	if err != nil {
		return err
	}
	return nil
...
func ProductUpdate(ctx context.Context, conn *pgx.Conn, product models.ProductModel) error {
	...
	_, err := conn.Exec(ctx, sqlQuery, product.ProductName, product.StorageID, product.ExpireDate, product.Weight, product.Article)
	if err != nil {
		return err
	}
	return nil
